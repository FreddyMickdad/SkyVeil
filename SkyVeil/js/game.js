...
preloadGame.prototype = {
  preload: function () {
    ...
    // No audio file loading - we'll generate sounds with Web Audio API
    console.log("Using Web Audio API for sound generation");
  },
  create: function () {
...
playGame.prototype = {
  ...
  playTone: function(frequency, duration, type = 'square', volume = 0.3) {
    if (!this.audioContext || !this.audioStarted) return;
  ...
  playGameOverSound: function() {
    ...
    setTimeout(() => this.playTone(200, 0.4, 'sawtooth', 0.4), 600);
  },
  
  startBackgroundMusic: function() {
    if (!this.audioContext || !this.audioStarted || this.bgMusicPlaying) return;
  ...
  playBackgroundLoop: function() {
    ...
    const notes = [523, 659, 784, 659]; // C-E-G-E progression
    const noteDuration = 0.4;
    
    notes.forEach((note, i) => {
      setTimeout(() => {
        if (this.bgMusicPlaying) {
          this.playTone(note, noteDuration, 'triangle', 0.15);
        }
      }, i * noteDuration * 1000);
    });
  ...
  createMenu: function () {
    ...
    var titleText = game.add.bitmapText(game.width / 2, tap.y - 200, "font", "SKYVEIL", 90);
  ...
  handleTap: function (pointer, doubleTap) {
    ...
    if (!this.audioStarted && this.soundsLoaded) {
